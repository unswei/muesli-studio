...
Current release using this target set: `v0.4.0`.
...
```bash
pnpm release:package -- --target linux-intel --version v0.4.0
pnpm release:package -- --target macos-arm --version v0.4.0
```

Bundle verification script:

```bash
pnpm release:verify-bundle -- dist/release/muesli-studio-v0.4.0-macos-arm.tar.gz
```

Detached signing script:

```bash
pnpm release:sign-archive -- dist/release/muesli-studio-v0.4.0-macos-arm.tar.gz
```
...
Then open the release page for `v0.4.0` and download the required artefact.
...
```bash
shasum -a 256 -c muesli-studio-v0.4.0-linux-intel.tar.gz.sha256
gpg --verify muesli-studio-v0.4.0-linux-intel.tar.gz.asc \
  muesli-studio-v0.4.0-linux-intel.tar.gz
```
